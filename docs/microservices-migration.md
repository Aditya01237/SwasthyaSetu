...
1. Extract `hospital-service` on port `8084`.
2. Extract `appointment-service` on port `8083` and add Redis slot locks.
3. Extract `patient-service` on port `8082`.
4. Add `notification-service` on port `8086` as RabbitMQ consumers.
