...
const StatCard = ({ icon, label, value, sub, color = "sky", delay = 0 }) => {
  const accents = {
    ...
    green:
      "from-emerald-500/20 to-teal-600/20 border-emerald-500/20 text-emerald-400",
    amber:
      "from-amber-500/20 to-yellow-600/20 border-amber-500/20 text-amber-400",
    violet:
      "from-violet-500/20 to-purple-600/20 border-violet-500/20 text-violet-400",
  };
  return (
    <div
      className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-5 flex flex-col gap-2
                 hover:border-sky-500/20 transition-colors duration-200 animate-fadeUp"
      style={{ animationDelay: `${delay}ms` }}
    >
      <div
        className={`w-10 h-10 rounded-xl bg-gradient-to-br border flex items-center justify-center text-lg ${accents[color]}`}
      >
        {icon}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#090c12] text-slate-200 font-sans">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 pt-10 pb-24">
        ...
        ══════════════════════════════════ */}
        <div
          className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-6 md:p-8 mb-6
                     flex flex-col md:flex-row items-start md:items-center gap-6 animate-fadeUp"
        >
          {/* Avatar */}
          <div className="relative flex-shrink-0">
            <div
              className="w-24 h-24 rounded-2xl bg-gradient-to-br from-sky-500/30 to-blue-700/30 border border-sky-500/25
                            flex items-center justify-center font-display text-4xl font-bold text-sky-400"
            >
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              {doctor.hospitalName && (
                <span className="flex items-center gap-1.5 text-slate-500 text-xs">
                  <svg
                    className="w-3.5 h-3.5 text-sky-500/70"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth={2}
                    viewBox="0 0 24 24"
          ...
          <div className="w-full md:w-auto flex-shrink-0 space-y-2.5 md:min-w-[180px]">
            <button
              ...
              className="w-full bg-gradient-to-r from-sky-500 to-blue-600 text-white text-sm font-medium
                               py-3 px-6 rounded-xl hover:opacity-90 hover:-translate-y-px active:translate-y-0
            ...
            <button
              className="w-full bg-white/[0.04] border border-white/[0.08] text-slate-300 text-sm font-medium
                               py-2.5 px-6 rounded-xl hover:bg-white/[0.07] transition-all duration-200
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            ...
            icon="👥"
            label="Patients"
            value={doctor.totalPatients || "500+"}
            sub="treated"
            color="violet"
          />
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          ...
          <div className="md:col-span-2 space-y-5">
            {/* About */}
            <div
              className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-6 animate-fadeUp"
            ...
            >
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                {doctor.about ||
                  `Dr. ${doctor.name} is a highly skilled ${doctor.specialization} with over
                  ${doctor.experience}+ years of clinical experience. Renowned for a patient-first
                  approach, precision in diagnosis, and consistently delivering exceptional
                  treatment outcomes. Trusted by hundreds of patients across ${doctor.hospitalName || "the region"}.`}
              </p>
            </div>
                ...
                {(
                  ...
                  <div
                    ...
                    className={`flex items-center justify-between px-4 py-3 rounded-xl border text-sm
                      ${
                        slot.time === "Closed"
                          ? "border-red-500/10 bg-red-500/[0.03] text-red-400/60"
          ...
          <div className="space-y-5">
            ...
            >
              ...
              <InfoRow
                label="Languages"
                value={doctor.languages || "English, Hindi"}
              />
            </div>

            {/* Qualifications */}
            {(
              doctor.qualifications || ["MBBS", "MD – " + doctor.specialization]
            )?.length > 0 && (
              <div
                className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-6 animate-fadeUp"
...
══════════════════════════════════ */}
      {showBooking && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-[#0f141c] border border-white/[0.08] rounded-2xl p-6 w-full max-w-md animate-fadeUp">
            <h2 className="font-display text-lg font-semibold text-slate-100 mb-4">
              Book Appointment
            </h2>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full mt-1 bg-white/[0.05] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-slate-200"
              />
            </div>

            {/* Slots */}
            <div className="mb-4">
              <label className="text-xs text-slate-500">Select Time Slot</label>

              <div className="grid grid-cols-3 gap-2 mt-2">
                {["10:00 AM", "11:00 AM", "12:00 PM", "4:00 PM", "5:00 PM"].map(
                  (slot) => (
                    <button
                      key={slot}
                ...
                ${
                  selectedSlot === slot
                    ...
                    : "bg-white/[0.03] border-white/[0.08] text-slate-400 hover:border-sky-500/30"
                }`}
                    >
                      {slot}
                    </button>
            ...
            <div className="flex gap-2 mt-6">
              <button
                ...
                className="flex-1 py-2 rounded-xl bg-white/[0.05] text-slate-400 hover:bg-white/[0.08]"
              >
                Cancel
              </button>
              ...
              <button
                ...
                className="flex-1 py-2 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 text-white hover:opacity-90"
              >
